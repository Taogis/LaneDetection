...
def test_lanenet(image_path, weights_path, use_gpu, image_list, batch_size, save_dir):
    ...
    image_list = test_dataset.get_img_list()
    ...
    with sess.as_default():
        ...
        for i in range(math.ceil(len(image_list) / batch_size)):
            ...
            for cnt, image_name in enumerate(paths):
                ...
                file_exist = open(os.path.join(save_dir, os.path.basename(image_name)[:-3] + 'exist.txt'), 'w')

                instance_result_image = (instance_seg_image[cnt, :, :, 0 + 1] * 255).astype(np.uint8)
                for cnt_img in range(4):
                    if cnt_img > 0:
                        instance_result_image = cv2.addWeighted(instance_result_image, 1, (instance_seg_image[cnt, :, :, cnt_img + 1] * 255).astype(np.uint8), 1, 0)
                    if cnt_img == 3:
                        orgin_img = cv2.imread(image_name, cv2.IMREAD_COLOR)
                        orgin_img = cv2.resize(orgin_img, (CFG.TRAIN.IMG_WIDTH,CFG.TRAIN.IMG_HEIGHT), interpolation=cv2.INTER_LINEAR)
                        instance_result_image = cv2.cvtColor(instance_result_image, cv2.COLOR_GRAY2RGB)
                        instance_result_image[:, :, 0] = 0
                        instance_result_image = cv2.addWeighted(orgin_img, 1, instance_result_image, 1, 0)
                        cv2.imwrite(os.path.join(save_dir, os.path.basename(image_name)[:-4] + '_' + str(cnt_img + 1) + '_avg.png'),instance_result_image)
                    #cv2.imwrite(os.path.join(save_dir,os.path.basename(image_name)[:-4] + '_' + str(cnt_img + 1) + '_avg.png'),
...
if __name__ == '__main__':
    # init args
    args = init_args()
    args.weights_path = '/home/mmtao/test/Codes-for-Lane-Detection/SCNN-Tensorflow/lane-detection-model/tools/model/culane_lanenet/culane_scnn/culane_lanenet_vgg_2019-02-14-09-04-47.ckpt-200000'
    args.batch_size = 1
    args.is_batch = 'false'
    args.image_path = '/home/mmtao/anzhi-2018-09-22-08-51-51-089'
    args.save_dir = '/home/mmtao/test/Codes-for-Lane-Detection/SCNN-Tensorflow/testResult'
    ...
    with open(str(args.image_path), 'r') as g:
        for line in g.readlines():
            img_name.append('/home/mmtao/data/CULane/' + line.strip())
    '''
